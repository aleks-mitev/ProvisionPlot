...
export default function ListScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      setIsFabReady(false); // Instantly lock the button when screen appears
      
      const timer = setTimeout(() => {
        setIsFabReady(true); // Unlock after 800ms
      }, 800);

      return () => clearTimeout(timer); // Cleanup if they leave the screen early
    }, [])
  );
  ...
  useFocusEffect(
    useCallback(() => {
      const onBackPress = () => {
        if (mode === 'logging') {
          router.push('/daily-log'); // Force route to Daily Log
          return true; // Tell React Native we handled the back press (prevents default behavior)
        }
        return false; // Let the normal back behavior happen if not logging
      };

      // Store the returned subscription object
      const subscription = BackHandler.addEventListener('hardwareBackPress', onBackPress);

      return () => {
        // Call .remove() directly on the subscription
        subscription.remove();
      };
    }, [mode])
  );
